...
class AqaraLightSwitchDouble extends HomeKitDevice {
  ...
  onDeviceReady() {
    ...
    this.getServiceCharacteristic('LeftButton', 'ProgrammableSwitchEvent').setProps(defaultButtonProps)
    this.getServiceCharacteristic('RightButton', 'ProgrammableSwitchEvent').setProps(defaultButtonProps)
    this.getServiceCharacteristic('BothButton', 'ProgrammableSwitchEvent').setProps(defaultButtonProps)
    const сlickParser = {
      cluster: 'genOnOff',
  ...
  handleClickReport(type, data, device, characteristic) {
    let timeoutKey = `${type}ClickTimeout`
