...
class HomeKitDevice {
  ...
  unstableStatusMountServiceCharacteristic({ endpoint, cluster, service, characteristic, ...options }) {    
    const serviceCharacteristic = this.getServiceCharacteristic(service, characteristic)
    if (options.status) {
      this.zigbee.unstableStatusSubscribe(endpoint, cluster, options.status, (data) => {
        serviceCharacteristic.updateValue(options.statusParser(data, this, serviceCharacteristic))
      })
    }
  }
